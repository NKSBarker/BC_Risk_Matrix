...
# =================
# UNFINISHED STUFF
# STILL TO DO: FIND A WAY OF PLOTTING THE ACTUAL RANK (FROM KARI'S MATRIX) ABOVE EACH BARPLOT TO ENABLE EASY COMPARISON.
# aLSO, PLOT RAW COUNTS AGAINST FOREST ATTRIBUTES TO COMPARE TO THE ESTIMATED MEAN DENSITY
# =================
...
``` {r}
load("RDataFiles/finalRanks.RData")
ranks.qs <- ranks
...
*As of August 20 2018, I'm a bit worried about whether I'm running the right analyses. Right now this function plots a density diagram of all rows in the totalBirds file. I think each row corresponds to an individual PKEY. There can be more than one PKEY in a given forest stand polygon. So I'm overrepresenting stands that have been sampled more. Nevermind the fact that some stand TYPES (forID) have been sampled more than others. I suspect I'll need to take a subset of this table and redo the plots below. *


#### Density as a function of rank 

``` {r}
ggplot(totalBirds, aes(x=as.factor(FinalRank), y=PredDens_ForID, group=as.factor(FinalRank), fill=as.factor(FinalRank))) + geom_boxplot() + xlab("Rank") + ylab("Mean Density per Forest Stand Type") + myfacettheme

ggplot(totalBirds, aes(x=as.factor(FinalRank), y=PredDens_ForID, group=as.factor(FinalRank), col=as.factor(FinalRank), fill=as.factor(FinalRank))) + geom_violin() + myfacettheme + xlab("Rank") + ylab("Mean Density per Forest Stand Type")
```
...
``` {r}
ggplot(totalBirds, aes(x=as.factor(FinalRank), y=Count, group=as.factor(FinalRank), fill=as.factor(FinalRank))) + geom_boxplot() + xlab("Rank") + ylab("Mean Density per Forest Stand Type") + myfacettheme

ggplot(totalBirds, aes(x=as.factor(FinalRank), y=Count, group=as.factor(FinalRank), col=as.factor(FinalRank), fill=as.factor(FinalRank))) + geom_violin() + myfacettheme + xlab("Rank") + ylab("Mean Density per Forest Stand Type")
...
``` {r eval=T}
ggplot(totalBirds, aes(x=ForAge, y=PredDens_ForID, fill=ForHt)) + geom_boxplot() + xlab("Forest Age") + ylab("Mean Density per Forest Stand Type") + myfacettheme

ggplot(totalBirds, aes(x=ForAge, y=PredDens_ForID, fill=ForHt, col=ForHt)) + geom_violin() + xlab("Forest Age") + ylab("Mean Density per Forest Stand Type") + myfacettheme
```
...
``` {r eval=T}
ggplot(totalBirds, aes(x=ForAge, y=Count, fill=ForHt)) + geom_boxplot() + xlab("Forest Age") + ylab("Mean Density per Forest Stand Type") + myfacettheme

ggplot(totalBirds, aes(x=ForAge, y=Count, fill=ForHt, col=ForHt)) + geom_violin() + xlab("Forest Age") + ylab("Mean Density per Forest Stand Type") + myfacettheme
```
...
``` {r eval=T}
tota.bec <- split(totalBirds, totalBirds$BEC)

lapply(tota.bec, function(x) {
  ggplot(x, aes(x=ForAge, y=PredDens_ForID, fill=ForHt)) + geom_boxplot() + facet_wrap(~BEC) + xlab("Forest Age") + ylab("Mean Density per Forest Stand Type") + myfacettheme
})
  
lapply(tota.bec, function(x) {
  ggplot(x, aes(x=ForAge, y=PredDens_ForID, col=ForHt, fill=ForHt)) + geom_violin()  + facet_wrap(~BEC)+ xlab("Forest Age") + ylab("Mean Density per Forest Stand Type") + myfacettheme
})
```
...
``` {r eval=T}
tota.sg <- split(totalBirds, totalBirds$SpeciesGroup)

lapply(tota.sg, function(x) {
  ggplot(x, aes(x=ForAge, y=PredDens_ForID, fill=ForHt)) + geom_boxplot() + facet_wrap(~SpeciesGroup) + xlab("Forest Age") + ylab("Mean Density per Forest Stand Type") + myfacettheme
})
  
lapply(tota.sg, function(x) {
  ggplot(x, aes(x=ForAge, y=PredDens_ForID, col=ForHt, fill=ForHt)) + geom_violin() + facet_wrap(~SpeciesGroup)+ xlab("Forest Age") + ylab("Mean Density per Forest Stand Type") + myfacettheme
})
```
...
lapply(tota.becsg, function(x) {
  ggplot(x, aes(x=ForAge, y=PredDens_ForID, fill=ForHt, col=ForHt, group=ForHt)) + geom_point(size=3, position=position_dodge(width=0.8)) + facet_wrap(~BEC.Species)+ xlab("Forest Age") + ylab("Mean Density per Forest Stand Type") + myfacettheme
})
