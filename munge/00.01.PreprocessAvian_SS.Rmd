...
``` {r load.xy}
xy<- data.table(read.csv(paste0(droproot, "BC_Risk_Matrix/BC_COFI_XY.csv"), header=T))
...
``` {r}
ss.covar <- data.table(read.csv(paste0(droproot, "BC_Risk_Matrix/covariatesforoffsetMay2017Timezone_manuallyFixed.csv"), header=T))
colnames(ss.covar)
