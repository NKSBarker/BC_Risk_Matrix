...
datadir <- "C:/Users/nbarker/Dropbox/BAM/BC_Risk_Matrix/data" 
repodir <- "C:/Users/nbarker/Documents/repos/BC_Risk_Matrix"
```
...
``` {r load.xy}
xy<- data.table(read.csv(paste0(datadir, "/BC_COFI_XY.csv")))
...
``` {r}
ss.covar <- data.table(read.csv(paste0(datadir, "/covariatesforoffsetMay2017Timezone_manuallyFixed.csv"), header=T))
colnames(ss.covar)
...
``` {r}
save(xy.ss.covar, file=paste0(repodir, "/cache/xy.ss.covar.RData"))
```