...
**SCRIPT OUTPUT : data.table in RData file xy.ss.covar.RData (see cache folder on Dropbox)**
...
``` {r load.xy}
xy<- data.table(read.csv(paste0(droproot, "data/BC_Risk_Matrix/BC_COFI_XY.csv"), header=T))
...
``` {r}
ss.covar <- data.table(read.csv(paste0(droproot, "data/BC_Risk_Matrix/covariatesforoffsetMay2017Timezone_manuallyFixed.csv"), header=T))
colnames(ss.covar)
...
``` {r}
save(xy.ss.covar, file=("cache/xy.ss.covar.RData"))
```

#### Write to table for Trish to quality-check missing data

``` {r}
write.csv(xy.ss.covar, file="output/BCdata_xy.ss.covar.RData")
```