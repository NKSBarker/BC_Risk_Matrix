...
2. **csv in output folder in repo for sharing with Trish (or others)**
3. **docx file with text and code snippets plus inline output - so you don't need to run this script**
...
``` {r}
write.csv(xy.ss.covar, file=paste0(droproot, "output/BCdata_xy.ss.covar.csv"))
```